 ...
 *  - Caches every result in a transient so the call happens at most once per
 *    unique image URL (cleared automatically when the attachment changes).
 */
function kotlinskidev_get_image_size_cached($src)
{
    $cache_key = 'img_size_' . md5($src);
    $cached = get_transient($cache_key);
    if ($cached !== false) {
        return $cached; // [] means "no size found" — still a valid cached result
    }
    ...
    if (strpos($src, $upload_url) === 0) {
        ...
        if (file_exists($file_path)) {
            ...
            $result = ($info && isset($info[0], $info[1]))
...
// Flush cached image sizes when an attachment is updated or deleted
add_action('edit_attachment', function($attachment_id) {
    $src = wp_get_attachment_url($attachment_id);
    if ($src) {
        delete_transient('img_size_' . md5($src));
    }
});
