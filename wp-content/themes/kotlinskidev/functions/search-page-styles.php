...
function kotlinskidev_is_search_page()
{
    ...
    if (is_page() && (get_post_field('post_name') === 'search' || strpos($request_uri, '/search/') !== false)) {
        return true;
    }
            ...
            if (!empty($all_content)) {
                ...
                if (function_exists('wp_style_engine_get_stylesheet_from_context')) {
                    ...
                    if (!empty($block_styles)) {
                        echo '<style id="wp-block-supports-inline-css">' . $block_styles . '</style>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSS generated by WP core's own style engine from parsed site content, not user input; escaping would corrupt the inline CSS
                    }
                }
            }
        }, 1);
