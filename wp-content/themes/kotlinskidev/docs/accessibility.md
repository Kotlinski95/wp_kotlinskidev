...
Scope: `src/scripts/*` (`accessibility.ts`, `hamburger.ts`, `mega-menu.ts`, `search-panel.ts`, `language-panel.ts`, `theme-switcher.ts`, `scroll-to-top.ts`), `src/styles/accessibility.scss`, `functions/image-link-accessibility.php`, `functions/polylang-accessibility.php`, `functions/link-hover-effects.php`, the 10 custom blocks in `src/blocks/*`, the 9 color-scheme variants (`styles/*.json`) × light/dark mode (`docs/theme-colors.md`), and the PL/EN Polylang routing.
