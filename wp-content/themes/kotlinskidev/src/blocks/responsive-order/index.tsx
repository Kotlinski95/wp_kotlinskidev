...
// Get breakpoints from localized script data
declare global {
    interface Window {
...
// Types
interface ResponsiveOrderAttributes {
    responsiveOrder?: {
        desktop?: number;
        tablet?: number;
        mobile?: number;
...
// Add responsive order attributes to all blocks
...
// Add responsive order controls to block inspector
const withResponsiveOrderControls = createHigherOrderComponent((BlockEdit) => {
    return (props: any) => {
        ...
        return (
            <Fragment>
                ...
                <InspectorControls>
                    <PanelBody
                        title={__('Responsive Order', 'kotlinskidev')}
                        initialOpen={false}
                    >
                        <NumberControl
                            ...
                            help={__('Set the order for desktop screens (' + breakpoints.desktop_min + 'px+). Use negative values like -1 for higher priority.', 'kotlinskidev')}
                            min={-1}
                            max={20}
                            step={1}
                        />
                        <NumberControl
                            ...
                            help={__('Set the order for tablet screens (' + breakpoints.tablet_min + 'px - ' + breakpoints.tablet_max + 'px).', 'kotlinskidev')}
                            min={-1}
                            max={20}
                            step={1}
                        />
                        <NumberControl
                            ...
                            help={__('Set the order for mobile screens (below ' + breakpoints.mobile_max + 'px).', 'kotlinskidev')}
                            min={-1}
                            max={20}
                            step={1}
                        />
...
}, 'withResponsiveOrderControls');    // Add custom CSS classes to blocks based on responsive order
    ...
    // Apply responsive order classes to saved content (THIS WAS MISSING!)
    function applyResponsiveOrderClass(extraProps: any, blockType: any, attributes: any) {
        const { responsiveOrder } = attributes;
...
// THIS IS THE MISSING FILTER - it ensures classes are saved to database!
addFilter(
