...
const toGalleryMedia = (item: WPMediaItem): GalleryMedia => {
  ...
  return {
    id: item.id,
    url: item.url,
    thumbnailUrl: isVideo ? poster : (item.url ?? item.sizes?.large?.url ?? item.sizes?.medium?.url ?? ''),
    alt: item.alt ?? '',
...
export default function Edit({
  ...
  const { videoControls = true, videoAutoplay = false, videoLoop = false, videoMuted = false } = attributes;
      ...
      <InspectorControls>
        <PanelBody title={__('Media', 'kotlinskidev')} initialOpen>
          <MediaUploadCheck>
              ...
              <div className="gallery-lightbox-sidebar-grid">
                {images.map((item) => (
                  <div key={item.id} className="gallery-lightbox-sidebar-item">
                    <img
                      src={item.type === 'video' ? (item.poster || item.thumbnailUrl) : item.thumbnailUrl}
                      alt={item.alt}
                    />
                    {item.type === 'video' && (
                      <span className="gallery-lightbox-sidebar-badge" aria-hidden="true">&#9654;</span>
                    )}
                    <button
            ...
            <MediaUpload
              ...
              render={({ open }) => (
                <Button
                  ...
                  onClick={open}
                  style={{ width: '100%', justifyContent: 'center', marginTop: images.length ? '0.5rem' : 0 }}
                >
                  {images.length === 0
            ...
            {useMobileMedia && (
              <>
                <p className="gallery-lightbox-sidebar-label">{__('Mobile media (≤768px)', 'kotlinskidev')}</p>
                ...
                <MediaUpload
                  ...
                  render={({ open }) => (
                    <Button
                      ...
                      onClick={open}
                      style={{ width: '100%', justifyContent: 'center', marginTop: mobileImages.length ? '0.5rem' : 0 }}
                    >
                      {mobileImages.length === 0
        ...
        <CarouselPanel
          ...
          features={{ arrowsPosition: true, navColor: true, navPlacement: true, trackActiveSlide: true }}
        />
        {hasVideos && (
          <PanelBody title={__('Video Options', 'kotlinskidev')} initialOpen={false}>
            ...
            <ToggleControl
              ...
              onChange={(value) => setAttributes({ videoAutoplay: value, videoMuted: value ? true : videoMuted })}
            />
            <ToggleControl
          ...
          {!first ? (
            ...
            <div className="gallery-lightbox-editor">
              <div className="gallery-lightbox-trigger" style={{ pointerEvents: 'none' }}>
                ...
                />
                {first.type === 'video' && (
                  <span className="gallery-lightbox-video-badge" aria-hidden="true">&#9654;</span>
                )}
                {images.length > 1 && (
