...
const MOBILE_BREAKPOINT: number =
  (window as any).kotlinskiTheme?.mobileBreakpoint ?? 767;
...
const buildModalHTML = (
  ...
  const { showArrows = true, showPagination = true, arrowsPosition = 'sides', navColor = '', navPlacement = 'inside',
    videoControls = true, videoAutoplay = false, videoLoop = false, videoMuted = false } = settings;

  const slides = images
    .map(
      ({ src, alt, type, poster }, i) =>
        type === 'video'
  ...
  const navStyle = navColor ? ` style="--carousel-nav-color: ${navColor}; --carousel-nav-border-color: ${navColor}"` : '';
  ...
  const insideNav = hasCustomNav && !isOutside ? arrowsHTML : showArrows && !hasCustomNav ? arrowsHTML : '';
...
const initGalleryLightbox = (gallery: HTMLElement): void => {
  ...
  const applyAspectRatioFromImage = (img: HTMLImageElement): void => {
    if (img.naturalWidth && img.naturalHeight) {
      ...
      img.addEventListener('load', () => applyAspectRatio(trigger, img.naturalWidth, img.naturalHeight), { once: true });
    }
  };
  ...
  if (gallery.dataset.mobileImages && window.matchMedia(`(max-width: ${MOBILE_BREAKPOINT}px)`).matches) {
    syncInitialTrigger();
  }
