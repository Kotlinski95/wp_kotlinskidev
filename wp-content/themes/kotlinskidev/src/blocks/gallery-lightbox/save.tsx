...
export default function Save({
  ...
  const { images = [], useMobileMedia = false, mobileImages = [], ...carouselSettings } = attributes;
  ...
  const imagesData = JSON.stringify(
    images.map(({ url, alt, type, poster, width, height }) => ({ src: url, alt, type: type ?? 'image', poster: poster ?? '', width: width ?? 0, height: height ?? 0 }))
  );
  const mobileImagesData = useMobileMedia && mobileImages.length
    ? JSON.stringify(mobileImages.map(({ url, alt, type, poster, width, height }) => ({ src: url, alt, type: type ?? 'image', poster: poster ?? '', width: width ?? 0, height: height ?? 0 })))
    : null;
  ...
  return (
    ...
    >
      <button
        ...
        style={first.width && first.height ? { aspectRatio: `${first.width}/${first.height}` } : undefined}
      >
        <img
        ...
        {totalCount > 1 && (
          <span className="gallery-lightbox-count" aria-hidden="true">
            {carouselSettings.trackActiveSlide ? (
              <><span className="gallery-lightbox-count__current">01</span>{` / ${String(totalCount).padStart(2, '0')}`}</>
            ) : (
              `+${rest.length}`
            )}
          </span>
