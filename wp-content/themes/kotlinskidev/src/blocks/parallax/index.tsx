...
const withParallaxControls = createHigherOrderComponent((BlockEdit) => {
    return (props: BlockEditProps) => {
        ...
        const handleParallaxToggle = (value: boolean) => {
            console.log('🎛️ Parallax Controls Debug: Toggle changed to:', value);
            console.log('🎛️ Parallax Controls Debug: Client ID:', props.clientId);
            
            setAttributes({ enableParallax: value });
            
            // Apply/remove parallax class to the block element
            setTimeout(() => {
                const blockElement = document.querySelector(`[data-block="${props.clientId}"]`) as HTMLElement;
                console.log('🎛️ Parallax Controls Debug: Found block element:', blockElement);
                
                if (blockElement) {
                    console.log('🎛️ Parallax Controls Debug: Current classes before:', Array.from(blockElement.classList));
                    
                    if (value) {
                        blockElement.classList.add('enable-parallax');
                        blockElement.setAttribute('data-parallax-speed', parallaxSpeed.toString());
                        console.log('✅ Parallax Controls Debug: Added enable-parallax class and speed attribute');
                    } else {
                        blockElement.classList.remove('enable-parallax');
                        blockElement.removeAttribute('data-parallax-speed');
                        console.log('✅ Parallax Controls Debug: Removed enable-parallax class and speed attribute');
                    }
                    
                    console.log('🎛️ Parallax Controls Debug: Current classes after:', Array.from(blockElement.classList));
                    console.log('🎛️ Parallax Controls Debug: All attributes after:', 
                        blockElement.getAttributeNames().map(name => `${name}="${blockElement.getAttribute(name)}"`));
                } else {
                ...
                <InspectorControls>
                    ...
                    >
                        <ToggleControl
                            ...
                            help={__('Creates a scrolling depth effect where the background moves slower than the content.', 'kotlinskidev')}
                        ...
                        {enableParallax && (
                            <RangeControl
                                ...
                                max={1.0}
                                step={0.1}
                                help={__('Lower values create stronger parallax effect. 0.1 = very slow, 1.0 = normal speed', 'kotlinskidev')}
                            />
                        )}
                    </PanelBody>
                </InspectorControls>
