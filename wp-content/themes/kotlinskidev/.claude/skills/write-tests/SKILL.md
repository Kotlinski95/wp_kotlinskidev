...
- `admin.createNewPost()` leaves a real `auto-draft` row every call even unsaved — normal WP behavior. Don't build per-spec cleanup for it; `npm run test:e2e:purge-drafts` already runs automatically after every `test:e2e` invocation (pass or fail) to sweep these, since WP-Cron's own 7-day reap is far slower than this suite accumulates them.
...
- `docs/treeview.md`: add the new spec/test file(s) under the `tests/` section.
- `CHANGELOG.md`: add a dated, 2-3 sentence entry under `[Unreleased]` → `### Added` — see `.claude/skills/sync-docs/SKILL.md` for the exact format and when a version bump is (and isn't) warranted.
